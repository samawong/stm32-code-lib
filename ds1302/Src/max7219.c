...
{
	HAL_GPIO_WritePin(GPIOA,GPIO_PIN_0,i?GPIO_PIN_SET:GPIO_PIN_RESET);
}
void Max7219_pinCS(uint8_t i)
{
	HAL_GPIO_WritePin(GPIOA,GPIO_PIN_1,i?GPIO_PIN_SET:GPIO_PIN_RESET);
}
void Max7219_pinDIN(uint8_t i)
{
	HAL_GPIO_WritePin(GPIOA,GPIO_PIN_2,i?GPIO_PIN_SET:GPIO_PIN_RESET);
}
...
void Write_Max7219_byte(uint8_t DATA)         
{
	    ...
	    for(i=8;i>=1;i--)
          {		  
            Max7219_pinCLK(0);
            Max7219_pinDIN(DATA&0x80); //&10000000, get the higher position is 0 or 1
            DATA=DATA<<1;
            Max7219_pinCLK(1);		//?????????
           }                                 
}
...
{ 
     Max7219_pinCS(0);
	    Write_Max7219_byte(add1);           //send address
      Write_Max7219_byte(dat1);           //send the data 
	   Max7219_pinCS(1);                        
}
/***
